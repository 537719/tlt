...
MODIF   22:42 02/02/2021 renomm‚ encoutstock.old.cmd pour laisser la place … un nouveau coutstock.cmd qui travaille sur la bdd standard
:debut
@echo off
...
:: Inutile ^^ car d‚j… fait par ailleurs
...
if not exist tarif.csv goto :errtarif
 ::erreur for %%I in (is_stock is_catalogue tarif)  do if %%~zI == 0 goto errsize
...
REM for %%I in (%0) do sqlite3 <..\bin\%%~nI.sql
@echo on
sqlite3 <..\bin\%~n0.sql
 pause
